...
function getModel() {
    ...
    // YOUR CODE HERE
    model.add(tf.layers.conv2d({kernelSize:3, filters:32, activation: "relu", inputShape:[28,28,1]}));
...
async function train(model, data) {
    ...
    const [trainXs, trainYs] = tf.tidy(() => {
        const b = data.nextTrainBatch(TRAIN_DATA_SIZE);
        return [
            b.xs.reshape([TRAIN_DATA_SIZE,28,28,1]),
            b.labels
        ];
    });
    ...
    const [testXs, testYs] = tf.tidy(() => {
        const b = data.nextTrainBatch(TEST_DATA_SIZE);
        return [
            b.xs.reshape([TEST_DATA_SIZE,28,28,1]),
            b.labels
        ];
    });
